...
export async function saveRegistration(data: Record<string, unknown>) { const registrations = await readStore(); const submission: RegistrationSubmission = { id: `reg-${Date.now()}`, createdAt: new Date().toISOString(), ...data }; registrations.unshift(submission); await writeStore(registrations); return submission; }

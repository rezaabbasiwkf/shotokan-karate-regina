...
class BrowserSession {
  ...
  async initialize() { const response = await this.request("/api/auth/csrf"); const data = await response.json(); this.csrf = data.csrfToken; assert(response.ok && this.csrf, "CSRF initialization"); }
  async json(route, method, body) { const response = await this.request(route, { method, headers: { "content-type": "application/json" }, body: JSON.stringify(body) }); const data = await response.json(); return { response, data }; }
}
